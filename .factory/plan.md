...
- [x] T7 (standard) — fetch.py: relative entry links resolved against the pre-redirect feed URL (Fixes #66)
  ...
  - pr: 86
- [x] T8 (standard) — fetch.py: config parsing loses feeds — '#' truncation without whitespace, and multi-line flow lists yield zero feeds (Fixes #65)
...
- [x] T13 (standard) — monitor.sh: triage prompt renders "DEEP-DIVE QUEUE: enabledopus." when deep-dive is on (Fixes #60)
  ...
  - pr: 91
- [x] T14 (standard) — fetch.py: no total deadline or size cap on feed fetches — a slow-drip feed stalls the whole run (Fixes #59)
  ...
  - pr: 92
- [R] T15 (standard) — demo-bundle.sh: a broken symlink in state/ or kb/ aborts the build, leaving a partial bundle (Fixes #58)
