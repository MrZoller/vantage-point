...
# so `x="$(cfg_get ...)"` is safe under set -e - including when the FILE is unreadable,
# not just when the key is missing. Without the `|| true` that promise held only for a
# readable file: awk's nonzero status would propagate into the assignment and kill the
# caller under set -e, which in bootstrap.sh meant dying before the failure trap was even
# armed. These readers cannot distinguish "absent key" from "unreadable file" anyway -
# both yield the empty string - so returning 0 for both is the consistent contract.
