...
b@y.com" "$(cfg_get_list output email_to "$cfg")"
  ...
  # ---- email_report fans out to every recipient (To: header + msmtp envelope) ----
  local funcs="$TMP/emailfuncs2.sh"
  awk '/^# ---- email delivery ----/{f=1} /^# Promote this run.s output/{f=0} f' \
    "$ROOT/bin/monitor.sh" > "$funcs"
  local report="$TMP/report2.md"
  printf '# Daily - 1 item\n\n* something\n' > "$report"
  local mbin="$TMP/mbin"; make_min_bin "$mbin"
  local eml="$TMP/multi.eml"
  ( set -e
    # shellcheck disable=SC2030,SC2031  # subshell-local env is intentional (isolation)
  ...
  # Both addresses reach msmtp as separate envelope recipients.
  assert_eq "both recipients passed to msmtp as separate args" "a@x.com
b@y.com" "$(cat "$eml.rcpt")"
  # ...and the To: header lists both, comma-joined.
  local to_hdr
  to_hdr="$(python3 - "$eml" <<'PY'
import sys, email
print(email.message_from_file(open(sys.argv[1]))["To"])
PY
)"
  assert_eq "To: header is the comma-joined recipient set" "a@x.com, b@y.com" "$to_hdr"
}
test_email_multi
